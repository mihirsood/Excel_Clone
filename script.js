...
$(".input-cell").click(function (e) {
  ...
  if ($(this).hasClass("selected") && (e.metaKey || e.ctrlKey)) {
...
function selectCell(ele, e, topCell, bottomCell, leftCell, rightCell) {
  ...
  if (e.metaKey || e.ctrlKey) {
    // mac has metaKey instead of ctrlKey
    let topSelected;
    if (topCell) {
...
function changeHeader([rowId, colId]) {
  ...
  addRemoveSelectFromtFontStyle(data, "bold");
  addRemoveSelectFromtFontStyle(data, "italic");
  addRemoveSelectFromtFontStyle(data, "underline");
...
function addRemoveSelectFromtFontStyle(data, property) {
  if (data[property]) {
...
$(".input-cell").mousemove(function (e) {
  ...
  if (e.buttons == 1) {
    // none held gives 0
    // left click held gives 1
    // right click held gives 2

    if (e.pageX > $(window).width() - 20 && !scrollXRStarted) {
      scrollXR();
    } else if (e.pageX < 20 && !scrollXLStarted) {
      scrollXL();
    }
...
function setStyle(ele, prop, key, value) {
  if ($(ele).hasClass("selected")) {
    $(ele).removeClass("selected");
    $(".input-cell.selected").css(key, "");
    // $(".input-cell.selected").each(function (index, data) {
    //     let [rowId, colId] = getRowCol(data);
    //     cellData[rowId - 1][colId - 1][prop] = false;
    // });
    updateCellData(prop, false);
  } else {
    $(ele).addClass("selected");
    $(".input-cell.selected").css(key, value);
    // $(".input-cell.selected").each(function (index, data) {
    //     let [rowId, colId] = getRowCol(data);
    //     cellData[rowId - 1][colId - 1][prop] = true;
    // });
    updateCellData(prop, true);
  }
}
  ...
  if (value != defaultProperties[property]) {
    $(".input-cell.selected").each(function (index, data) {
      ...
      if (cellData[selectedSheet][rowId - 1] == undefined) {
        ...
        // ... is used for sparsing an object
...
function addSheetEvents() {
  $(".sheet-tab.selected").on("contextmenu", function (e) {
    // function gets called whenever contextmenu event is called
    e.preventDefault();
    selectSheet(this);
  ...
  $(".new").click(function (e) {
    if (saved) {
      ...
      $(".container").append(`<div class="sheet-modal-parent">
                        <div class="sheet-delete-modal">
                            <div class="sheet-modal-title">Excel - Book</div>
                            <div class="sheet-modal-detail-container">
...
function newFile() {
  ...
  cellData = { Sheet1: {} };
  $(".sheet-tab").remove();
  $(".sheet-tab-container").append(`<div class="sheet-tab">Sheet1</div>`);
  addSheetEvents();
...
function saveFile(newFileClicked) {
  $(".container").append(`<div class="sheet-modal-parent">
                                <div class="sheet-rename-modal">
...
function openFile() {
  ...
  // adding event listener which triggers whenever any change occurs
  inputFile.change(function (e) {
    ...
    // inbuilt function to read a file
    let reader = new FileReader();
    reader.readAsText(file);

    // functions executes when file reading completes
    reader.onload = () => {
      ...
      // 3 - cell data -  Json.parse(reader.result)
      cellData = JSON.parse(reader.result);

      // 4 - fill new sheets into sheet tabs
      ...
      // 5 - add event listener to the new Sheet
      addSheetEvents();
...
$("#formula-input").blur(function (e) {
  if ($(".input-cell.selected").length > 0) {
    ...
    for (let i of tempElements) {
      if (i.length >= 2) {
        // cell name has length atleast 2 ex. A1
        i = i.replace("(", "");
        i = i.replace(")", "");
  ...
  if (!cellData[selectedSheet][rowId - 1]) {
    ...
    if (update) {
      for (let i of oldUpstream) {
        ...
        if (
          JSON.stringify(cellData[selectedSheet][calRowId - 1][calColId - 1]) ==
          JSON.stringify(defaultProperties)
        ) {
        ...
        index =
          cellData[selectedSheet][rowId - 1][colId - 1].upStream.indexOf(i);
